classdef CInterface < handle
  ...
  methods
    ...
    function update(obj, varargin)
      ...
      assert(isempty(Px) || isempty(Px_idx) || length(Px) == length(Px_idx), 'inputs ''Px'' and ''Px_idx'' must be the same size');
      assert(isempty(Ax) || isempty(Ax_idx) || length(Ax) == length(Ax_idx), 'inputs ''Ax'' and ''Ax_idx'' must be the same size');
    ...
    function codegen(obj, target_dir, varargin)
      ...
      osqp_mex('codegen', obj.objectHandle, codegen_dir, p.Results.prefix, defines);
      ...
      if ~exist(codegen_src_dir, 'dir')
        codegen_src_dir = fullfile(osqp_root, 'build', '_deps', 'osqp-build', 'codegen_src');
      end
      if ~exist(codegen_src_dir, 'dir')
      ...
      if exist(codegen_src_dir, 'dir')
        if exist(fullfile(codegen_src_dir, 'src'), 'dir')
          copyfile(fullfile(codegen_src_dir, 'src'), fullfile(target_dir, 'src'));
        end
        if exist(fullfile(codegen_src_dir, 'inc'), 'dir')
          copyfile(fullfile(codegen_src_dir, 'inc'), fullfile(target_dir, 'inc'));
        end
        if exist(fullfile(codegen_src_dir, 'Makefile'), 'file')
          copyfile(fullfile(codegen_src_dir, 'Makefile'), fullfile(target_dir, 'Makefile'));
        end
      end
    end
    ...
    function adjoint_derivative_compute(obj, dx, dy)
      ...
      osqp_mex('adjoint_derivative_compute', obj.objectHandle, double(full(dx(:))), double(full(dy(:))));
    end
  ...
  methods (Access = private)
    function theSettings = validateSettings(~, isSetup, varargin)
      ...
      if isa(varargin{1}, 'osqp.SolverOptions')
        ...
        for k = 1:numel(fn)
          name = fn{k};
          if isfield(classDefaults, name) && ~isequaln(userSettings.(name), classDefaults.(name))
      ...
      if isfield(theSettings, 'linear_solver')
        theSettings.linsys_solver = osqp.CInterface.string_to_linsys_solver(theSettings.linear_solver);
        theSettings = rmfield(theSettings, 'linear_solver');
      elseif isfield(theSettings, 'linsys_solver') && (ischar(theSettings.linsys_solver) || isstring(theSettings.linsys_solver))
        theSettings.linsys_solver = osqp.CInterface.string_to_linsys_solver(theSettings.linsys_solver);
      end
      ...
      if ~isSetup
        ...
        for k = 1:numel(fnames)
          if ismember(fnames{k}, setupOnly)
            error('OSQP:update_settings', 'Setting ''%s'' cannot be updated after setup.', fnames{k});
          end
        end
      end
    end
